...
apps_activity_feed_consumer_export_variables() {
  ...
  export ACTIVITY_FEED_CONSUMER_HELM_DIR="$_helm_dir"
  ...
  # Templates
  export ACTIVITY_FEED_CONSUMER_SVC_MAP_TMPL="$_tmpl_dir/svc_map.yaml"
  export ACTIVITY_FEED_CONSUMER_HELM_VALUES_TMPL="$_tmpl_dir/values.yaml"
  ...
  export ACTIVITY_FEED_CONSUMER_DEPLOY_YAML="$_kubectl_dir/deploy.yaml"
  ...
  export ACTIVITY_FEED_CONSUMER_HELM_VALUES_YAML="$_helm_values_yaml"
...
apps_activity_feed_consumer_check_directories() {
  ...
  for _d in "$ACTIVITY_FEED_CONSUMER_HELM_DIR" \
    "$ACTIVITY_FEED_CONSUMER_KUBECTL_DIR"; do
    [ -d "$_d" ] || mkdir "$_d"
  done
}
...
apps_activity_feed_consumer_clean_directories() {
  ...
  for _d in "$ACTIVITY_FEED_CONSUMER_HELM_DIR" \
    "$ACTIVITY_FEED_CONSUMER_KUBECTL_DIR" \
...
apps_activity_feed_consumer_sh() {
  _deployment="$1"
  _cluster="$2"
  apps_activity_feed_consumer_export_variables "$_deployment" "$_cluster"
  _app="activity-feed-consumer"
...
apps_activity_feed_consumer_install() {
  ...
  # files
  _helm_values_tmpl="$ACTIVITY_FEED_CONSUMER_HELM_VALUES_TMPL"
  _helm_values_yaml="$ACTIVITY_FEED_CONSUMER_HELM_VALUES_YAML"
  _svc_map_tmpl="$ACTIVITY_FEED_CONSUMER_SVC_MAP_TMPL"
  ...
  # Image settings
  _image_repo="${ACTIVITY_FEED_CONSUMER_IMAGE%:*}"
  _image_tag="${ACTIVITY_FEED_CONSUMER_IMAGE#*:}"
  ...
  sed \
    ...
    -e "s%__ACTIVITY_FEED_CONSUMER_IMAGE_REPO__%$_image_repo%" \
    -e "s%__ACTIVITY_FEED_CONSUMER_IMAGE_TAG__%$_image_tag%" \
...
apps_activity_feed_consumer_helm_history() {
  _deployment="$1"
  _cluster="$2"
  apps_activity_feed_consumer_export_variables "$_deployment" "$_cluster"
  _app="activity-feed-consumer"
...
apps_activity_feed_consumer_helm_rollback() {
  _deployment="$1"
  _cluster="$2"
  apps_activity_feed_consumer_export_variables "$_deployment" "$_cluster"
  _app="activity-feed-consumer"
...
apps_activity_feed_consumer_command() {
  ...
  env-update | env_update)
    ...
    ;;
  helm-history)
    apps_activity_feed_consumer_helm_history "$_deployment" "$_cluster"
    ;;
  helm-rollback)
    apps_activity_feed_consumer_helm_rollback "$_deployment" "$_cluster"
    ;;
  ...
  sh) apps_activity_feed_consumer_sh "$_deployment" "$_cluster" ;;
