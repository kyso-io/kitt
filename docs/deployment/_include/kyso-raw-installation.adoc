...
Kyso requires **https**. In this section we are going to prepare the configuration for SSL Certificates. First of all, let's create the namespace in which
we will deploy the ingress
...
[IMPORTANT]
=========
**DECISION POINT**. You must choose which certificates are you going to use: custom certificates (generic, can be used with any provider) or AWS CERT Manager
=========

+
[IMPORTANT]
=========
**DECISION POINT**. You must choose which ingress are you going to use: classic LB (generic, compatible across cloud providers) or AWS NLB
=========

==== Custom certificates

Follow this instructions to configure kyso to use custom certificates. These are the prerrequisites necessary
...
This component is used to send notifications to users using **slack**.

. Create the slack-notifications-consumer namespace
...
. In kitt folder, open the folder `lib/kitt/tmpl/apps/slack-notifications-consumer`
...
metadata:
  name: elasticsearch
  namespace: slack-notifications-consumer-prod
spec:
  type: ExternalName
  externalName: elasticsearch-master.elasticsearch-prod.svc.cluster.local
  ports:
...
replicaCount: 1
image:
    repository: registry.kyso.io/kyso-io/consumers/slack-notifications-consumer
    tag: 2.0.20
    pullPolicy: Always
imagePullSecrets:
    - name: dockerconfigjson
...
[source,console]
----
helm upgrade --install -n slack-notifications-consumer-prod -f values.yaml slack-notifications-consumer $KITT_HOME/lib/kitt/charts/slack-notifications-consumer
----
...
[source,console]
----
$ kubectl get pods -n slack-notifications-consumer-prod
NAME                           READY   UP-TO-DATE   AVAILABLE   AGE
slack-notifications-consumer   1/1     1            1           18s
----
...
. Create the teams-notification-consumer namespace
...
. In kitt folder, open the folder `lib/kitt/tmpl/apps/teams-notification-consumer`
...
metadata:
  name: elasticsearch
  namespace: teams-notification-consumer-prod
spec:
  type: ExternalName
  externalName: elasticsearch-master.elasticsearch-prod.svc.cluster.local
  ports:
...
[source,console]
----
helm upgrade --install -n teams-notification-consumer-prod -f values.yaml teams-notification-consumer $KITT_HOME/lib/kitt/charts/teams-notification-consumer
----
...
[source,console]
----
$ kubectl get pods -n teams-notification-consumer-prod
NAME                                           READY   STATUS    RESTARTS   AGE
teams-notification-consumer-747d7bc49c-wp7bp   1/1     Running   0          14s
----
...
. In kitt folder, open the folder `lib/kitt/tmpl/apps/kyso-nbdime`
...
replicaCount: 1
image:
    repository: registry.kyso.io/docker/kyso-nbdime
    tag: latest
    pullPolicy: Always
imagePullSecrets:
    - name: dockerconfigjson
...
[source,console]
----
helm upgrade --install -n kyso-nbdime-prod -f values.yaml kyso-nbdime $KITT_HOME/lib/kitt/charts/kyso-nbdime
----
...
[source,console]
----
$ kubectl get pods -n kyso-nbdime-prod
NAME                          READY   STATUS    RESTARTS   AGE
kyso-nbdime-764c575dd-kqwcc   1/1     Running   0          16s
----
...
|KYSO_NBDIME_URL
|URL of the nbdime service
|http://kyso-nbdime.kyso-nbdime-prod.svc.cluster.local:3005
|=== 


=== Install **jupyter-diff microfrontend**

. Create the jupyter-diff namespace
...
. In kitt folder, open the folder `lib/kitt/tmpl/apps/jupyter-diff`
...
[source,console]
----
helm upgrade --install -n jupyter-diff-prod -f values.yaml jupyter-diff $KITT_HOME/lib/kitt/charts/jupyter-diff
----
...
[source,console]
----
$ kubectl get pods -n jupyter-diff-prod
NAME                          READY   STATUS    RESTARTS   AGE
kyso-nbdime-764c575dd-kqwcc   1/1     Running   0          16s
----

. Check that the microfrontend is accesible at https://<your_server>/kyjupdiff/